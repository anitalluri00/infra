...
=======
.PHONY: validate-k8s validate-scripts validate-terraform deploy-platform deploy-workloads bootstrap-tf-backend all-check
...
<<<<<<< HEAD
=======
	bash -n terraform/scripts/bootstrap_backend.sh
>>>>>>> 6f3b0ea (Local changes before pull)
...
bootstrap-tf-backend:
	@if [ -z "$(ENV)" ]; then \
		echo "Usage: make bootstrap-tf-backend ENV=prod-primary"; \
		exit 1; \
	fi
	bash terraform/scripts/bootstrap_backend.sh $(ENV)
>>>>>>> 6f3b0ea (Local changes before pull)
