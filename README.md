...
=======
1. Configure `terraform.tfvars` in:
   - `terraform/envs/prod-primary/`
   - `terraform/envs/prod-dr/`
   - Set a real `app_jwt_public_key` value in `terraform/envs/prod-primary/terraform.tfvars`
2. Bootstrap Terraform backend and initialize state:
   - `make bootstrap-tf-backend ENV=prod-primary`
   - `make bootstrap-tf-backend ENV=prod-dr`
3. Deploy infrastructure:
   - `cd terraform/envs/prod-primary && terraform apply`
   - `cd terraform/envs/prod-dr && terraform apply`
4. Deploy platform services (ingress/monitoring/logging/tracing/security):
